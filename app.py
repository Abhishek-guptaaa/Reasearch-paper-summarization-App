...
def main():
    ...
    llm = st.sidebar.selectbox("LLM",["gemma-7b-it", "gemma2-9b-it", "Other (open source in the future)"])
    ...
    if pdf_file_path != "":
        ...
        elif llm=="gemma2-9b-it":
            llm = ChatGroq(model_name="gemma2-9b-it",temperature=temperature)
        else:
            st.write("Using llm while open source models are not implemented!")
            llm = ChatGroq(temperature=temperature)
